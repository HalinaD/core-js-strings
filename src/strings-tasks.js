...
function repeatString(str, times) {
  return typeof str === 'string' && times > 0 ? str.repeat(times) : '';
}
...
function sumOfCodes(str) {
  if (typeof str === 'string') {
    const splitStr = str.split('').map((char) => char.charCodeAt(0));
    const sum = splitStr.reduce((a, b) => a + b, 0);
    return sum;
  }
  return 0;
}
...
function isPalindrome(str) {
  const removeSymbol = /[\W_]/g;
  const lowStr = str.toLowerCase().replace(removeSymbol, '');
...
function unbracketTag(str) {
  const remove = /<(.+)>/;
  const removeSym = str.match(remove);
  return removeSym ? removeSym[1] : '';
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-z]/gi, (word) => encrypt[alpha.indexOf(word)]);
}
